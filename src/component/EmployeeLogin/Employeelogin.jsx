...
function EmployeeLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      await axios
        .post(`${serverLink}/auth/employee/login`, credentials)
        .then((res) => {
          response = res;
        })
        .catch((error) => {
      ...
      if (response.status === 200) {
        ...
        if (keepLogged) {                           // Save data only if the user has checked keep me looged in button
          localStorage.setItem("employeeData", JSON.stringify(tokenData));
        }
